...
/**
  ...
  * @returns
  */
function getDoctorInfo (url: string, worksheet: any) {
...
function getData (html: any) {
  ...
  const list:any = [];
  // 表头处理
  $('.tableth th').each(function (index:number, ele:any) {
    list.push($(ele).text().trim());
  });

  data.push(list);
  let facultyName = '';
  $('.h5-table tr').each(function (index:number, ele:any) {
    const result:any = [];
    $(ele).children().each(function (idx:number, el:any) {
